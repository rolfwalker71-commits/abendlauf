...
PYTHONPATH=/tmp/ok-libs python3 werkzeuge/portraits-zuschneiden.py FOTO.jpg   # OK-Porträt einheitlich zuschneiden (Anleitung im Kopf der Datei)
...
- **Laufabend heute** (`js/renntag.js`): Leiste unten auf allen Seiten („Heute“ → „Jetzt am Start“ → „Unterwegs“, bei Status abgesagt/verschoben den ganzen Tag eine Warnung) und auf der Startseite der Zeitplan `#renntag-plan` mit markierter Startzeit. `base.html.twig` gibt dafür `#renntag-daten` aus (Laufabende der Startseite + Startzeiten der Ausschreibung), solange ein Abend bevorsteht; ob etwas erscheint, rechnet der Browser (Seiten-Cache). Testen mit simulierter Uhr (Playwright `clock.install`), nicht mit geänderten Inhalten.
...
- **Plugin-Texte auf Deutsch** stehen in `user/themes/abendlauf/languages.yaml` (Cookie-Hinweis `PLUGIN_CONSENT`) – unter `de` und per Verweis unter `en`, weil Grav ohne Mehrsprachigkeit unter `en` nachschlägt. Im Theme, damit sie beim Deployment mitkommen. Das Consent-Plugin ist seit September 2026 bewusst ausgeschaltet (`enabled: false`, keine Statistik- oder Werbedienste); die Texte bleiben für den Fall, dass es wieder eingeschaltet wird.
